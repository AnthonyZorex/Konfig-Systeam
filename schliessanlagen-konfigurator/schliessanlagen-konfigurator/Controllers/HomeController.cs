    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> SystemInfo(int id)
        {
            ...
            else
            {
                ...
                var Gallery = db.ProductGalery.Where(x => x.SysteamPriceKeyId == SystemSchois.Id).ToList();

                var listN = Gallery.Where(x=>x.SysteamPriceKeyId== SystemSchois.Id).Select(x => x.ImageName).ToList();

                ViewBag.Galery = listN.First();
            }
        ...
        List<string> Hebel, List<string> Vorhang, List<string> Aussen, List<IFormFile> postedFile,List<string> Options, List<string> ImageNameOption, List<string> Descriptions, List<int> inputCounter,
        ...
        public async Task<IActionResult> Edit_Doppelzylinder(Profil_Doppelzylinder profil_Doppelzylinder, string data,int DoppelId)
        {
            ...
            if (data == null)
            {
                ...
                var SizeDoppel = db.Aussen_Innen.Include(x => x.Doppel_Innen_klein).Include(x=>x.Doppel_Aussen_klein).Where(x => x.Profil_DoppelzylinderId == profil_Doppelzylinder.Id) .ToList();
                ...
                var kleinInnenDoppel = SizeDoppel.SelectMany(x => x.Doppel_Aussen_klein).ToList();

                if (kleinInnenDoppel.Count > 0)
                {
                    ViewBag.KleinInnen = SizeDoppel.Where(x => x.Doppel_Aussen_klein.Count > 0).ToList();
                }
        ...
        List<float> aussen_priesDoppelKlein, List<float> aussenDoppelKlein, List<float> innenKleinPreis, List<float> innenKlein, List<int> KleinZiseCountInter)
        {
            ...
            if (innenKlein.Count() > 0)
            {
                for (int i = 0; i < innenKlein.Count(); i++)
                {
                    var ausse_innenklein = new Aussen_Innen
                    {
                        Profil_DoppelzylinderId = Items.Id,
                        Intern = innenKlein[i],
                        costSizeIntern = innenKleinPreis[i],
                        costSizeAussen = 0,
                        aussen = 0
                    };
                    db.Aussen_Innen.Add(ausse_innenklein);
                    ...
                    for (int f = 0; f < KleinZiseCountInter[i]; f++)
                    {
                        var DoppelInternKlein = new Doppel_Aussen_klein
                        {
                            Aussen_InnenId = ausse_innenklein.Id,
                            aussen = aussenDoppelKlein[kleinStepIntern],
                            costSizeAussen = aussen_priesDoppelKlein[kleinStepIntern]
                        };
                        db.Doppel_Aussen_klein.Add(DoppelInternKlein);
                        db.SaveChanges();
                        kleinStepIntern++;
                    }
                }
        ...
        public async Task<IActionResult> Edit_Profil_Knaufzylinder(Profil_Knaufzylinder profil_Halbzylinder)
        {
            ...
            var SizeDoppel = db.Aussen_Innen_Knauf.Include(x => x.Aussen_Innen_Knauf_klein).Include(x=>x.Aussen_Knauf_klein).Where(x => x.Profil_KnaufzylinderId == profil_Halbzylinder.Id).ToList();
            ...
            var kleinInnenDoppel = SizeDoppel.SelectMany(x => x.Aussen_Knauf_klein).ToList();

            if (kleinInnenDoppel.Count > 0)
            {
                ViewBag.KleinInnen = SizeDoppel.Where(x => x.Aussen_Knauf_klein.Count > 0).ToList();
            }
        ...
        #region SaveEditForm
        [HttpPost]
        public async Task<IActionResult> SaveHalbzylinder(Profil_Halbzylinder profil_Halbzylinder, List<int> Size, List<float> CostSize, List<string> Options, List<string> ImageNameOption, List<IFormFile> postedFile,
        ...
        [HttpPost]
        public async Task<IActionResult> SaveHebelzylinder(Hebel profil_Halbzylinder, List<string> Options,List<string> ImageNameOption, List<IFormFile> postedFile,
        ...
        [HttpPost]
        public async Task<IActionResult> SaveAussenzylinder_Rundzylinder(Aussenzylinder_Rundzylinder profil_Halbzylinder, List<string> Options, List<string> ImageNameOption, List<IFormFile> postedFile,
        ...
        [HttpPost]
        public async Task<IActionResult> SaveVorhangschloss(Vorhangschloss profil_Halbzylinder,List<int>Size,List<float> CostSize, List<string> Options, List<string> ImageNameOption, List<IFormFile> postedFile,
        List<string> Descriptions, List<string> valueNGF, List<float> costNGF, List<int> inputCounter, List<IFormFile> UploadGalleryImages, List<string> GalleryImages)
        {
